...
float get_reward(float rssi_metric, int state, int action);
...
void process(){
    ...
    for(int slotframe = 0; slotframe < LIMIT; slotframe++){
        ...
        for(int timeslot=0; timeslot<SLOTFRAME_SIZE; timeslot++){
            ...
            reward = get_reward(rssi_metric,timeslot,action);
...
int main(){
    ...
    file_actions = fopen("outputs/actions.txt","w");
    file_outputs = fopen("outputs/metric-outputs.txt","w");
    file_formatted_channels_availability = fopen("outputs/formatted-channels-availability.txt","w");
    file_number_of_chosen_action_per_slot = fopen("outputs/number-of-chosen-action-per-slot.txt","w");
